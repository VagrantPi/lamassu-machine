...
import { dbFormat, save, triggerWithCustomInfo } from './fixtures'

import { loadConfig, loadMachineInfo, saveTerms, saveConfig } from '../../../../lib/storage/settings/settings'
...
describe('on empty db', () => {
  describe('load config', () => {
    ...
    test.for([
      ...
      ['triggersAutomation', {}],
      ['terms', { active: false }],
      ['operatorInfo', { active: false }],
      ['staticConfig', undefined],
    ])('%s should be empty',
      ([source, expected]) => expect(config[source]).toEqual(expected))
  })
