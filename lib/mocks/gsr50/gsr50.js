...
function handleCommand (msg) {
  const denomination = msg.denomination
  switch (msg.command) {
    case 'insertBill':
...
Gsr50.prototype.run = function run (cb, { cassettes, recyclers, rejectedDenominations }) {
  this.isRejectedDenomination = Object.fromEntries(rejectedDenominations.map((denom) => [denom, true]))
  this.getDeviceState()
...
Gsr50.prototype.hasRejectedDenominations = function hasRejectedDenominations (bills) {
  return bills.some(({ denomination }) => this.isRejectedDenomination[denomination])
}

Gsr50.prototype.emitEvent = function emitEvent(denomination) {
  let destinationUnit = _.find(it => it.denomination.toNumber() === denomination.toNumber() && it.count < 60)(this.recyclersCounts)
  if (destinationUnit)
    this.recyclersCounts[destinationUnit.number].count++
  destinationUnit = destinationUnit?.name ?? 'cashbox'

  const bills = [{ denomination, destinationUnit }]
