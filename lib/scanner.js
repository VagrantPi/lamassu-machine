...
const scanPDF417 = (callback, idCardStillsCallback) => {
  ...
  const saveFailedScans = idCardStillsCallback ?
    dirs => idCardStillsCallback(dirs).then(() => Promise.all(_.map(rmrf, dirs))) :
    () => Promise.resolve(null)
  ...
  maybeTmpdir(idCardStillsCallback)
    .then(tmpdir => {
      const tmpdirs = tmpdir ? [tmpdir] : []
