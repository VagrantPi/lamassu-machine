...
const reduceToKeyValue = (arr, key, value) => arr.reduce(
  (ret, item) => Object.assign(ret, { [item[key]]: item[value] }), {},
)
...
const loadTriggers = (trx) => {
  ...
  return trx('triggers')
    ...
    .then(async rows => {
      ...
      const choicesByInput = choiceLists.reduce((acc, { customInput, choiceText }) => {
        acc[customInput] = acc[customInput] || []
        acc[customInput].push(choiceText)
        return acc
      }, {})
