...
Trader.prototype.saveConfig = function saveConfig (staticConfigChanged, termsChanged) {
  ...
  if (!staticConfigChanged)
    return lssettings.saveTerms(this.termsHash, this.terms).catch(err => console.log("Failed saving terms on lssettings:", err))
  ...
  }).catch(err => console.log("Failed saving config on lssettings:", err))
}
