...
const { loadMachineInfo } = require('../storage/settings')
...
Updater.prototype.updateHeaders = function updateHeaders (headers) {
  ...
  const machineInfoPromise = loadMachineInfo().catch(() => ({ active: false }))

  return Promise.all([machineInfoPromise, getXrandrDetails(isCC), getLightDMStatus(isCC)])
    .then(([machineInfo, xrandrDetails, lightdmStatus]) => {
